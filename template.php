 ...
 */
function corolla_preprocess_page(&$variables) {
  // Add $main_links variable to page.tpl.php. Unlike built-in $main_menu variable, it supports nested menus
  if (isset($variables['main_menu'])) {
    $pid = variable_get('menu_main_links_source', 'main-links');
    $tree = menu_tree($pid);
    $variables['main_links'] = drupal_render($tree);
  } else {
...
function corolla_css_alter(&$css) {
  ...
  unset($css[drupal_get_path('module', 'contextual') . '/contextual-rtl.css']);
...
function corolla_js_alter(&$js) {
  unset($js[drupal_get_path('module', 'contextual') . '/contextual.js']);
}
 ...
 * Simplify the pager
 */
function corolla_pager($variables) {
  $tags = $variables['tags'];
